...
namespace cl_util {
    ...
    matrix::matrix(size_t rows, size_t cols) : rows(up(rows)), cols(up(cols)), data(
            static_cast<float *>(malloc(this->cols * this->rows * sizeof(float)))) {
    }

    void matrix::fill_rand(float rand_min, float rand_max) const {
    ...
    matrix transpose(matrix const& orig) {
        matrix res(orig.rows, orig.cols);;
    ...
    bool compare(const matrix &a, const matrix &b, float eps) {
        ...
        for (size_t i = 0; i < a.rows; i++) {
            for (size_t j = 0; j < b.cols; j++) {
                ...
                if (diff > eps) {
                    std::cerr << std::fixed << std::setprecision(10) << "Failed compare. Actual: "
                              << b.data[i * b.cols + j] << " Expected: " << a.data[i * b.cols + j]
